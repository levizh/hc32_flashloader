...
void CLK_HRCInit(void)
{
    ...
    /* enable HRC */
    M4_CMU->HRCCR = 0;
    while(0 == bM4_CMU->OSCSTBSR_b.HRCSTBF);
...
void CLK_PLLInit(void)
{
    ...
    if (1UL == (*((volatile unsigned int*)(0x404UL)) & 1UL))
    {
        ...
        PQR=1(div2) -->240MHz
        */
        M4_CMU->PLLCFGR = (1UL<<28) | (1UL<<24) | (1UL<<20) | (59UL << 8) | (1UL << 7) | (1UL);
    }
    /* HRC = 20MHz */
    else
    {
        ...
        PQR=1(div2) -->240MHz
        */
        M4_CMU->PLLCFGR = (1UL<<28) | (1UL<<24) | (1UL<<20) | (47UL << 8) | (1UL << 7) | (1UL);
    }
                    ...
                    char const *buffer)
{
    ...
    */
//    M4_EFM->FWMC = (0UL << 8) | (1UL<<4);

    M4_EFM->FWMC |= (1UL << 8) | (3UL<<4);
//    M4_EFM->FWMC = (0UL << 8) | 1UL;
    ...
    do
    {
        *((volatile flashunit *)LoadAddr) = *pData;
        ...
        u32To = 0UL;
        while (EFM_FSR_OPTEND != (M4_EFM->FSR & EFM_FSR_OPTEND))
//        while (EFM_FSR_RDY != (M4_EFM->FSR & EFM_FSR_RDY))
        {
            u32To++;
            if (u32To >= TIMEOUT)
            {
        ...
        M4_EFM->FSCLR |= EFM_FSCLR_OPTENDCLR;
...
uint32_t _FlashPrepare(void)
{
    ...
    */
    M4_EFM->FRMC = (1<<19) | (1<<0);

    /*
    Bus hold disable? enable
    */
...
uint32_t _FlashEraseSector(uint32_t SectorAddr)
{
    ...
    *((volatile flashunit *)SectorAddr ) = 0 ;
    ...
    /* clear opend flag, it's necessary */
    M4_EFM->FSCLR |= EFM_FSCLR_OPTENDCLR;
    /* read only mode */
